...
latest_frames = {CAM_CHEST_ID: None, CAM_WRIST_ID: None}
frame_timestamps = {CAM_CHEST_ID: [], CAM_WRIST_ID: []}
captured_frames = {CAM_CHEST_ID: 0, CAM_WRIST_ID: 0}
written_frames = {CAM_CHEST_ID: 0, CAM_WRIST_ID: 0}
video_writers = {CAM_CHEST_ID: None, CAM_WRIST_ID: None}
...
# Camera worker
# =========================
def get_video_size(cam_id):
    rotation = CAM_ROTATION.get(cam_id, 0)
...
def camera_worker(cam_id):
    ...
    while is_running:
        ret, frame = cap.read()
        if ret:
            frame = rotate_frame(frame, CAM_ROTATION.get(cam_id, 0))
...
def save_metadata(session_dir):
    metadata = {
        ...
        "imu_hz": 200,
        "frames": {"captured": captured_frames, "written": written_frames}
    }
...
def start_record(session_dir):
    ...
    with writer_lock:
        for cam_id, folder in [(CAM_WRIST_ID, "camera_0"), (CAM_CHEST_ID, "camera_1")]:
            w, h = get_video_size(cam_id)
            video_writers[cam_id] = cv2.VideoWriter(
                os.path.join(session_dir, folder, "raw_video.mp4"),
